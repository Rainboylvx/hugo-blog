...
treeSize tree = go [tree] 0
  where
    ...
    go (Branch _ l r : rest) acc = go (l : r : rest) (acc + 1)
```
...
```haskell
treeSize :: Tree a -> Int
treeSize = foldTree (\_ l r -> 1 + l + r)
  where
    foldTree :: (a -> Int -> Int -> Int) -> Tree a -> Int
    foldTree _ Empty          = 0
    foldTree f (Branch x l r) = f x (foldTree f l) (foldTree f r)
```
