...
插件之间松耦合通信靠事件（Cordis 的通信核心机制）。事件分**监听**与**触发**两端：
...
> [!IMPORTANT] waterfall 监听器**必须调用 next()**。不调用 next 会短路整个流水线，这是故意为之的设计，用于实现拦截或网关逻辑。策略监听器在拥有决策权时，可以不调用 next() 直接返回；仅做标注或观察的监听器则必须委托给下游。
...
// 此后 ctx.on / ctx.emit 的参数会被自动推断，拼错事件名或参数类型都会在编译期报错
```
...
> `turn/*`、`step/*`、`tool/call`、`tool/result` 是**持久化的会话事件类型，不是同名 Cordis 事件**。需要观察它们时，监听 `session/event` 并检查 `event.type`。
...
- waterfall 必须调用 `next()`；不调用即短路，是设计而非 bug。
- `declare module` 合并让事件名与参数有类型保障。